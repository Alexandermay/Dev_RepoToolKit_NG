...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <OAI-PMH xmlns="http://www.openarchives.org/OAI/2.0/">
            <ListRecords>
                <xsl:for-each select="collection('../../Dev_RepoToolKit_NG/TempRepo/xml/collection.xml')">
                    <record>
                        <metadata>
                            <mira_import>
            ...
            <xsl:when test="//AbstractSection[@ID]/Heading">
                <dc:abstract>
                    <xsl:value-of select=".//AbstractSection[@ID][1]/Heading"/>: <xsl:value-of
                        select="substring-before(.//AbstractSection[@ID][1]/Para[1],'.')"/><xsl:text>.</xsl:text>
                </dc:abstract>
            </xsl:when>
            <xsl:when test="//Abstract/Heading">
                <dc11:abstract>
                    <xsl:value-of select=".//Abstract/Heading"/>: <xsl:value-of
                        select="substring-before(.//Abstract[1]/Para[1],'.')"/></dc11:abstract><xsl:text>.</xsl:text>
            </xsl:when>
            <xsl:otherwise/>
        </xsl:choose>
    </xsl:template>
    <xsl:template name="description">
        <xsl:choose>
            <xsl:when test="//AbstractSection[@ID]/Heading">
                <dc11:description>
                    <xsl:value-of select=".//AbstractSection[@ID][1]/Heading"/>: <xsl:value-of
                        select="normalize-space(.//AbstractSection[@ID][1]/Para[1])"/>
                </dc11:description>
            </xsl:when>
            <xsl:when test="//Abstract/Heading">
                <dc11:description>
                    <xsl:value-of select=".//Abstract/Heading"/>: <xsl:value-of
                        select="normalize-space(.//Abstract[1]/Para[1])"/>
                </dc11:description>
            </xsl:when>
            <xsl:otherwise/>
        </xsl:choose>
