...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <OAI-PMH xmlns="http://www.openarchives.org/OAI/2.0/">
            <ListRecords>
                <xsl:for-each
                    select="collection('../../Dev_RepoToolKit_NG/TempRepo/collection.xml')/root/row">
                    <record>
                        <metadata>
                            <mira_import>
    ...
    <xsl:template match="Visibility" name="visibility">
        <xsl:choose>
            <xsl:when test="Visibility[contains(text(), 'Auth')]">
    ...
    <xsl:template match="Format" name="has_model">
        <xsl:choose>
            ...
            <xsl:when test="Format = 'text/html'">
                <model:hasModel>GenericObject</model:hasModel>
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <dc:bibliographicCitation>
                    <xsl:value-of select="replace(normalize-space(Source), '&lt;/?\w+[^&lt;]*&gt;', '')"/>
                </dc:bibliographicCitation>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="License" name="license">
        <xsl:choose>
            <xsl:when test="License[contains(text(),text())]">
